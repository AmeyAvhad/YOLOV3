...
def build_targets(model, targets):
    ...
    targets = targets.to(device)  # Ensure targets tensor is on the same device as the model
    ...
    for i in model.yolo_layers:
        ...
        t, a = targets, []
        gwh = t[:, 4:6] * ng
        gwh = gwh.to(device)  # Ensure gwh tensor is on the same device as the model
...
def plot_images(imgs, targets, paths=None, fname='images.png'):
    ...
    ns = int(bs ** 0.5)  # number of subplots
